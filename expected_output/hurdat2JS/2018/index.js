import Alberto from './Alberto'
import Beryl from './Beryl'
import Chris from './Chris'
import Debby from './Debby'
import Ernesto from './Ernesto'
import Florence from './Florence'
import Gordon from './Gordon'
import Helene from './Helene'
import Isaac from './Isaac'
import Joyce from './Joyce'
import Eleven from './Eleven'
import Kirk from './Kirk'
import Leslie from './Leslie'
import Michael from './Michael'
import Nadine from './Nadine'
import Oscar from './Oscar'

const hurdat2_2018 = [Alberto, Beryl, Chris, Debby, Ernesto, Florence, Gordon, Helene, Isaac, Joyce, Eleven, Kirk, Leslie, Michael, Nadine, Oscar]

export default hurdat2_2018import Alberto from './Alberto'
import Beryl from './Beryl'
import Chris from './Chris'
import Debby from './Debby'
import Ernesto from './Ernesto'
import Florence from './Florence'
import Gordon from './Gordon'
import Helene from './Helene'
import Isaac from './Isaac'
import Joyce from './Joyce'
import Eleven from './Eleven'
import Kirk from './Kirk'
import Leslie from './Leslie'
import Michael from './Michael'
import Nadine from './Nadine'
import Oscar from './Oscar'

const hurdat2_2018 = [Alberto, Beryl, Chris, Debby, Ernesto, Florence, Gordon, Helene, Isaac, Joyce, Eleven, Kirk, Leslie, Michael, Nadine, Oscar]

export default hurdat2_2018